...
export async function GET(request: NextRequest, { params }: { params: Params }) {
  try {
    ...
    let cartProducts = await db.collection('products').find({ id: { $in: cartDoc.cartIds } }).toArray();
    if (cartProducts.length === 0) {
      const legacyDb = client.db('ecommenrce-nextjs');
      cartProducts = await legacyDb.collection('products').find({ id: { $in: cartDoc.cartIds } }).toArray();
    }
    return new Response(JSON.stringify(cartProducts), { status: 200, headers: { 'Content-Type': 'application/json' } });
  } catch (err) {
    console.error('GET /api/users/:id/cart error:', err);
    return new Response(JSON.stringify([]), { status: 200, headers: { 'Content-Type': 'application/json' } });
  }
}
